...
class rechargePaymentDriver extends PaymentDriver
{
    public function CreateLink($payment, $parameter)
    {
        ...
        $html .= '<div class="field" style="width:400px;"><label>充值卡密码：</label><input type="password" name="password" class="f_input l_input" /></div>';
        $html .= '<p style="padding-left:15px;"><b>注意：实际充值金额是您的充值卡面额</b></p>';
        $html .= '<div class="clear"></div><div class="act" id="l_act"><input type="submit" class="formbutton formbutton_invite" value=" 充 值 " /></div>';
        $html .= '</form>';
