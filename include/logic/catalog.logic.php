...
class CatalogLogic
{
	...
	public function Navigate_index () {		/******2012-08-29-为让首页不显示秒杀分类，写的单独函数******/
		$class = $this->GetList_index();
		if (!$class) return array();
		foreach ($class as $i => $topclass)
		{
			$class[$i]['subclass'] = $this->GetList_index($topclass['id']);
		}
		$this->ProCountSync();
		return $class;
	}
	...
	public function GetList_index($parent = 0)		/******2012-08-29-为让首页不显示秒杀分类，写的单独函数******/
	{
		return dbc(DBCMax)->select('catalog')->where('parent='.$parent.'and parent!="miaosha"')->done();
	}
