...
class MasterObject
{
	...
	function MasterObject(&$config)
	{
		...
		$this->Server  = &$_SERVER;
		$this->Files   = &$_FILES;
		$this->Module = trim($this->Post['mod']?$this->Post['mod']:$this->Get['mod']);	//获取mod值，并设置Module值为mod值
		$this->Code   = trim($this->Post['code']?$this->Post['code']:$this->Get['code']);	//功能同上
		$this->OPC   = trim($this->Post['op']?$this->Post['op']:$this->Get['op']);	//功能同上
		
		if ($this->Code == '')
		{
			$this->Code = ini('settings.default_code');	//ini函数在./include/engine.php
		}
		
				global $rewriteHandler;
		$__navs = ConfigHandler::get('nav');	//类ConfigHandler在./include/lib/config.han.php文件中
		foreach ($__navs as $i => $nav)	//头部导航
		{
			$curClass = '';
